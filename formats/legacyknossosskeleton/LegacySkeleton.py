...
from GeneralUtilities.Versions import Version
from GeneralUtilities.Checksum import integer_checksum
from GeneralUtilities.Coordinates import euclidian_distance
import copy
import zipfile
import os
import tempfile
...
class LegacySkeleton:
    ...
    def getNodes(self):
        ...
        # very bad line below, adding O(n^2)
        #all_nodes = all_nodes.union(cur_annotation.getNodes())
            ...
            for anno_index, e_dict in enumerate(edges):
                ...
                #2 0 16.022520 33.760513 7.047000 0.463378 1
                    ...
                    if (len(rev_edges[anno_index][src_n]) == 0) and not \
                            root_set:
                        #print('n id: ' + str(src_n.getUniqueID()))
                        # set the root
                        root_set = True
    ...
    def fromDom(self, doc, use_file_scaling=False,  scaling='dataset', comment=None, meta_info_only=False):
       ...
       # Construct annotation. Annotations are trees (called things inside the nml files).
        ...
        for annotation_elem in annotation_elems:
            annotation = LegacySkeletonAnnotation().fromNml(
                    annotation_elem,
                    self,
                    base_id=base_id)
            if comment:
        ...
        return self
    #
    def fromNmlcTree(self, filename, use_file_scaling=False, scaling='dataset', comment=None):
        ...
        for annotation_elem in annotation_elems:
            annotation = LegacySkeletonAnnotation().fromNmlcTree(
                    annotation_elem,
                    self,
                    base_id=base_id)
    ...
    def to_kzip(self, filename, save_empty=True, force_overwrite=False):
        """
        Similar to self.toNML, but writes LegacySkeleton to k_zip.
        ...
        if os.path.isfile(filename):
            try:
                ...
                print "Couldn't open file for reading and overwriting."
        else:
            try:
                with zipfile.ZipFile(filename, "w", zipfile.ZIP_DEFLATED) as zf:
                    zf.writestr('annotation.xml', self.to_xml_string(save_empty))
    ...
    def to_xml_string(self, save_empty=True):
        ...
        # build_attributes(offset, [["x", 0], ["y", 0], ["z", 0]])
        # parameters.appendChild(offset)
...
class LegacySkeletonAnnotation:
    ...
    def fromNml(self, annotation_elem, skeleton, base_id=0):
        ...
        for node_elem in node_elems:
            node = LegacySkeletonNode().fromNml(self, node_elem)
            self.addNode(node)
            #print node.getID()
        #
        # Read edges
    ...
    def fromNmlcTree(self, annotation_elem, skeleton, base_id=0):
        """ Subfunction of fromNmlcTree from LegacySkeleton

        annotation_elem -- XML Element
        skeleton -- type LegacySkeleton()
        """
        self.resetObject()
        ...
        for node_elem in node_elems:
            node = LegacySkeletonNode().fromNmlcTree(self, node_elem)
            self.addNode(node)
    ...
    def removeEdge(self, node, target_node):
        ...
        #    pass
        # try:
        #     self.edges[target_node].remove(node)
        # except KeyError:
        #     pass
        #try:
        ...
        #    pass
        # try:
        #     self.reverse_edges[node].remove(target_node)
        # except KeyError:
        #     pass
        return
    #
...
class LegacySkeletonNode:
    ...
    def from_scratch(self, annotation, x, y, z, inVp=1, inMag=1, time=0, ID=None, radius=1.0):
        ...
        #self.setDataElem("x", x)
        #self.setDataElem("y", y)
        #self.setDataElem("z", z)
    ...
    def getCoordinate(self):
        ...
        #return [self.getDataElem("x"), self.getDataElem("y"),
        # self.getDataElem("z")]
    #
    def setCoordinate(self, coord):
        ...
        #self.setDataElem("x", coord[0])
        #self.setDataElem("y", coord[1])
        #self.setDataElem("z", coord[2])
    #
    def getCoordinate_scaled(self):
        return [self.x_scaled, self.y_scaled, self.z_scaled]
        #return [self.getDataElem("x") * self.annotation.scaling[0], \
        #        self.getDataElem("y") * self.annotation.scaling[1], \
        #        self.getDataElem("z") * self.annotation.scaling[2]]
    #
    def getID(self):
